...
type Props = {
  variant: 'create' | 'edit';
  initialValues?: InitialValues; // edit에서만 필수로 넣어도 됨
  onSubmit: (body: {
    title: string;
    contents: string;
    hashtags?: string[];
  }) => Promise<{ id: string } | void>; // create는 id 반환, edit은 void여도 됨
};
...
export default function QuestionForm({
  ...
  // ✅ edit에서 initialValues가 늦게 들어오는 경우(로딩 후) 반영
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col w-full font-sans max-w-270">
      ...
      <div className="flex flex-col gap-2 mt-6">
        <h1 className="text-neutral-text-strong text-display-32">
          {variant === 'create' ? '질문 등록' : '질문 수정'}
        </h1>
